...
CDB.FirebugDB = extend(CDB.Module,
{
    ...
    {
        // Get only failures that happend in the last 5 days.
        var startDate = dateFrom.getTime();
        var endDate = dateTo.getTime();

        var options = {
            startkey: startDate,
            endkey: endDate,
            //group: true, // grouping can't be used for non reduce views
            success: function(data) {
                callback(data);
            }
        };
